...
- (void)viewDidLoad
{
    ...
    self.showViewOffset = self.showViewOffsetScale * ScreenWidth;
}
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    ...
    if (previousViewController) {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            ...
            [shotInfo setSafeObject:contentImage forKey:contentImageKey];
            [shotInfo setSafeObject:barImage forKey:barImageKey];
            [shotInfo setSafeObject:[NSValue valueWithCGRect:previousViewController.view.frame] forKey:contentFrameKey];
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    if (touchPoint.x < 0 || touchPoint.y < 10 || touchPoint.x > ScreenWidth - 100.) return NO;
